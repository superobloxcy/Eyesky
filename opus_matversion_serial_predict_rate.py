...
home = [home_lat, home_lon, home_alt]  # Home position (lat, lon, alt)
...
# To store the last known state of the aircraft
last_aircraft_state = {
    "lat": 0.0,
    "lon": 0.0,
    "alt_m": 0.0,
...
def update_plot_data(actual_lat, actual_lon, predicted_lat, predicted_lon, pred_time_used):
    ...
    with plot_data_lock:
        ...
        # We'll check it when we receive actual data at time = now + pred_time_used
        if pred_time_used > 0:
        ...
        # Look for predictions made pred_time_used seconds ago
        ...
        for pred_timestamp, (pred_lat, pred_lon, pred_dt) in prediction_buffer.items():
            ...
            # The prediction was for time: pred_timestamp + pred_dt
            # Current time is: absolute_time
            # So we check if absolute_time ≈ pred_timestamp + pred_dt
...
def prediction_updater():
    """
    This function runs in a separate thread. It continuously calculates
    the predicted position and sends it to the serial port at a high rate (100ms).
    """
    while True:
        with aircraft_state_lock:
            # Copy the state to avoid holding the lock for too long
            state = last_aircraft_state.copy()
        ...
        # Time elapsed since the last *actual* position was received
        time_since_last_actual = time.time() - state["timestamp"]

        # Predict this far into the future from the last actual position
        total_prediction_delta = current_prediction_time + time_since_last_actual

        future_lat, future_lon, future_alt_m = get_future_position(
            state["lat"], state["lon"], state["alt_m"],
            state["track_deg"], state["speed_kts"], state["vert_rate_fpm"],
        ...
        if not (azalt[1] < -30 or azalt[1] > 54):
            send_position(f"AZ:{azalt[0]:.5f} ALT:{azalt[1]:.5f}")

        # --- Update GUI ---
        # This function will update the red dot and predicted lat/lon labels
        update_gui_with_prediction(future_lat, future_lon)

        # Loop every 100ms
        time.sleep(0.1)


def update_gui_with_prediction(predicted_lat, predicted_lon):
    """
    Update the GUI with the latest predicted position.
    This is called from the high-frequency prediction_updater thread.
    """
    global current_predicted
    with plot_data_lock:
        current_predicted = (predicted_lat, predicted_lon)
        # Also update the list used for drawing the predicted path
        if predicted_positions:
            # To avoid growing the deque from this thread, just update the last point
            # The main loop will still append to it, this just keeps the line fresh
            predicted_positions[-1] = (predicted_lat, predicted_lon, time.time() - start_time if start_time else 0)
...
def site_poller(driver):

    """

    This function runs in a separate thread. It polls the website for the

    actual aircraft position at a low rate (500ms).
    ...
    while True:
        ...
        try:
            ...
            position = pos.find_element(By.XPATH, '//*[@id="selected_position"]')

            height = pos.find_element(By.XPATH, '//*[@id="selected_altitude_geom1"]')

            track = pos.find_element(By.XPATH, '//*[@id="selected_track1"]')

            gspd = pos.find_element(By.XPATH, '//*[@id="selected_speed1"]')

            vertspd = pos.find_element(By.XPATH, '//*[@id="selected_vert_rate"]')



            acp = string_to_float(position.text)

            ach_text = re.findall(r"(-?\d+\.?\d*)", height.text)

            act_text = re.findall(r"(-?\d+\.?\d*)", track.text)

            acs_text = re.findall(r"(-?\d+\.?\d*)", gspd.text)

            acv_text = re.findall(r"(-?\d+\.?\d*)", vertspd.text)



            if not all([acp, ach_text, act_text, acs_text, acv_text]):

                print("Incomplete data from website, retrying...")
            ...
            act = float(act_text[0])

            acs = float(acs_text[0])

            acv = float(acv_text[0])

            ach = float(ach_text[0])
            ...
            with aircraft_state_lock:
                ...
                last_aircraft_state["lat"] = acp[0]

                last_aircraft_state["lon"] = acp[1]

                last_aircraft_state["alt_m"] = achm

                last_aircraft_state["track_deg"] = act

                last_aircraft_state["speed_kts"] = acs

                last_aircraft_state["vert_rate_fpm"] = acv

                last_aircraft_state["timestamp"] = time.time()

            

            print(f'Polled Site: Lat: {acp[0]} Lon: {acp[1]}')



            # --- Update Plotting and Error Calculation ---

            # Get the current prediction time for error calculation purposes
            ...
            # We need a predicted position to log against the actual one.

            # We can calculate one here based on the new data.

            # This is NOT sent to the mount, just for logging.

            log_pred_lat, log_pred_lon, _ = get_future_position(
            ...
            update_plot_data(acp[0], acp[1], log_pred_lat, log_pred_lon, current_prediction_time)



        except Exception as e:

            print(f"Error in site poller: {e}")
...
# Start the high-frequency prediction updater thread
...
# Start the low-frequency site polling thread

poller_thread = threading.Thread(target=site_poller, args=(driver,), daemon=True)
...
# The GUI thread is not enough if the user closes the window.
