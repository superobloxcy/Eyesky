...
void processPacket(String packet) {
  ...
  if (azIndex != -1 && altIndex != -1) {
    ...
    if (!isEmergencyStopped) {
      ...
      Serial.printf("[Serial] Targeting -> AZ: %.3f, ALT: %.3f\n", newAz, newAlt);
...
void setup() {
  Serial.begin(115200);
  delay(1000);
  Serial.println("\n\n=== ESP32 Telescope Controller (Serial Mode) ===");
  ...
  Serial.println("Motors locked. Listening for serial data...");
...
void loop() {
  ...
  // 4. Timeout check - if no valid position received within timeout period
  if (millis() - lastPacketTime > POSITION_TIMEOUT) {
    // Optional: stop motors or take other safety action
    // For now, just continue operating
  }
}
