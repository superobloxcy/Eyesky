...
# Android USB port configuration
USB_PORT = "/dev/bus/usb/001/002"
BAUD_RATE = 115200
...
def send_position(cmd):
    """
    Sends a command to the ESP32 via USB serial port on Android.
    ...
    if not connected:
        print(f"Attempting to connect to ESP32 on {USB_PORT} at {BAUD_RATE} baud...")
        try:
            # Ensure any previous broken serial connection is closed before creating a new one
            if ser:
                ser.close()
            ser = serial4a.Serial(USB_PORT, BAUD_RATE, timeout=5)
